#!/usr/bin/env node
/**
 * Test suite for BlakeUI Skill scripts (JavaScript/Node.js).
 *
 * Usage:
 ...
 *     - production: https://mcp-api.blakeui.com
 *
...
const SCRIPTS_DIR = join(__dirname, "..", "..", "skills/blakeui-react/scripts");
const NODE = "node";
...
async function testListComponents(apiBase, verbose = false) {
  ...
  const envVars = {BLAKEUI_API_BASE: apiBase};
...
async function runTests(env, scripts = null, verbose = false) {
  ...
  console.log(`\n${"=".repeat(60)}`);
  console.log(`BlakeUI Skill Scripts Test Suite (JavaScript)`);
