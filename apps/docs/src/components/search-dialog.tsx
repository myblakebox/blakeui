...
import {Chip, Kbd, Tag, TagGroup} from "@blakeui/react";
...
const PRO_URL = process.env["NEXT_PUBLIC_PRO_URL"] ?? "https://blakeui.pro";
...
export default function CustomSearchDialog(props: SharedProps) {
  ...
  const proResults: SearchItemType[] = useMemo(() => {
    ...
    return proComponents.map((comp) => ({
      ...
      onSelect: () =>
        window.open(
          `${PRO_URL}${comp.slug}?utm_source=blakeui_docs&utm_medium=search&utm_campaign=pro_components`,
          "_blank",
        ),
