...
import {Chip, Tooltip, toast} from "@blakeui/react";
...
/**
 * Resolves the live color value from the rendered theme instead of relying on a
 * hard-coded string, so tooltips always reflect the actual theme variables
 * (and stay in sync with accent/theme switching).
 *
 * - For derived tokens (a `color-mix(...)` formula in `tooltip`) the formula is
 *   kept and the resolved value is appended.
 * - For everything else the tooltip shows `--token: oklch(...)` from the DOM.
 */
function useResolvedTooltip<T extends HTMLElement = HTMLDivElement>(
  variable: string,
  tooltip?: string,
) {
  const ref = React.useRef<T>(null);
  const [content, setContent] = React.useState<string | undefined>(tooltip);
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      const resolved = toOklchCss(background);
      const token = getTokenName(tooltip, variable);

      if (tooltip?.includes("color-mix")) {
        setContent(`${tooltip}\n= ${resolved}`);
      } else {
        setContent(`${token}: ${resolved}`);
      }
    };
...
function ColorHeader({
  ...
  const {content, ref} = useResolvedTooltip(bgVariable, tooltip);
  const token = getTokenName(content, bgVariable);

  return (
    <ColorTooltip content={content}>
      <div
        ref={ref}
...
function ColorBlock({
  ...
  return (
    <ColorTooltip className="block w-full" content={content}>
      <div
        ref={ref}
...
function SoftHoverSwatch({
  ...
  const {content, ref} = useResolvedTooltip(variable, tooltip);
  const token = getTokenName(content, variable);
  ...
  return (
    <ColorTooltip className="block w-full" content={content}>
      <div
        ...
        style={{backgroundColor: cssValue || `var(${variable})`}}
        ...
        {!!soft && (
          <div className="relative flex flex-1 flex-col gap-1.5 overflow-hidden rounded-xl p-3">
            ...
            <div className="relative">
              <SoftHoverSwatch
                cssValue={soft.hoverCssValue}
                tooltip={soft.hoverTooltip}
                variable={soft.hoverVariable}
              />
            </div>
            <div className="relative">
              <ColorBlock
...
function StackedSwatch({
  ...
  return (
    <ColorTooltip className="min-w-0 basis-full sm:flex-1 sm:basis-0" content={content}>
      <div
        ref={ref}
        style={{backgroundColor: bgValue}}
        className={cn(
...
function FormFieldThemeBlock({colors, theme}: {colors: FormFieldColors; theme: "light" | "dark"}) {
  const {content: bgContent, ref: bgRef} = useResolvedTooltip<HTMLDivElement>(
    colors.bg,
    colors.bgTooltip,
  );
  const {content: placeholderContent, ref: placeholderRef} = useResolvedTooltip<HTMLDivElement>(
    colors.placeholder,
    colors.placeholderTooltip,
  );
  const {content: foregroundContent, ref: foregroundRef} = useResolvedTooltip<HTMLDivElement>(
    colors.foreground,
    colors.foregroundTooltip,
  );
    ...
    <div data-theme={theme}>
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          >
            <ColorTooltip content={bgContent}>
              <span
          ...
          <div className="flex min-w-0 flex-row gap-2 sm:w-40 sm:flex-col">
            <ColorTooltip className="min-w-0 flex-1" content={placeholderContent}>
              <div
                ref={placeholderRef}
            ...
            <ColorTooltip className="min-w-0 flex-1" content={foregroundContent}>
              <div
                ref={foregroundRef}
