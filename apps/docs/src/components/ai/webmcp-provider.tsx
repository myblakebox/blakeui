...
const tools: WebMCPTool[] = [
  {
    description:
      "Search blakeUI React and Native documentation. Use this when an agent needs relevant blakeUI docs pages by keyword.",
    execute(input) {
    ...
    name: "search_blakeui_docs",
  },
  {
    description: "Retrieve a blakeUI documentation page as markdown by same-origin docs URL.",
    execute(input) {
    ...
    inputSchema: {
      properties: {
        url: {
          description: "A blakeUI docs path, for example /docs/react/components/button.",
          type: "string",
        },
      },
      required: ["url"],
    ...
    name: "get_blakeui_doc",
  },
  {
    description:
      "Navigate the browser to a blakeUI same-origin page such as the homepage or a docs page.",
    execute(input) {
      const url = resolveSameOriginPath(getString(input, "url"));

      if (!url) {
        throw new Error("url must be a same-origin blakeUI homepage, docs, or llms path");
      }
    ...
    inputSchema: {
      properties: {
        url: {
          description: "A same-origin blakeUI path, for example /docs/react/getting-started.",
          type: "string",
        },
      },
      required: ["url"],
    ...
    name: "navigate_blakeui",
  },
  {
    description: "List blakeUI component documentation pages for React or Native.",
    execute(input) {
  ...
  useEffect(() => {
    ...
    const registerTools = (modelContext: WebMCPModelContext | undefined): boolean => {
      ...
      try {
        ...
        if (!didWarn) {
          console.warn("Unable to register blakeUI WebMCP tools", error);
          didWarn = true;
        }
      }
