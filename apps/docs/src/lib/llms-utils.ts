...
import {isHiddenPage} from "@/lib/source";
...
export function filterExcludedPages(pages: Page[]): Page[] {
  return pages.filter((page) => page.slugs[0] !== "openapi" && !isHiddenPage(page));
}
...
export function generateIndexHeader(): string[] {
  ...
  return [
    "# blakeUI Documentation",
    "",
    "> A set of beautiful, customizable React and React Native components that stay maintained and up to date.",
    "",
    "blakeUI is an open-source UI component library for building modern web and mobile applications. Built on [Tailwind CSS v4](https://tailwindcss.com/) and [React Aria Components](https://react-spectrum.adobe.com/react-aria/), blakeUI provides accessible, customizable components with smooth animations and polished details.",
    "",
    "**Key Features:**",
    "",
...
export function generatePlatformIndexHeader(platform: Platform): string[] {
  ...
  return [
    `# blakeUI ${platformName} Documentation`,
    "",
    `> Documentation for blakeUI ${platformName} component library.`,
    "",
    `blakeUI ${platformName} is a component library built on ${techStack}. Every component comes with smooth animations, polished details, and built-in accessibility—ready to use, fully customizable.`,
    "",
    "**Key Features:**",
    "",
