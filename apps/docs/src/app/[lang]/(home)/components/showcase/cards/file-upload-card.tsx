...
export function FileUploadCard() {
  ...
  /* State swaps crossfade (~200ms) inside a layout parent so the card height
   * glides between the empty and file views instead of jumping. popLayout
   * takes the exiting view out of flow so both fade simultaneously.
   * Reduced motion: no animation props → instant swaps. */
  const crossfade = reducedMotion
    ? {}
    : {
        animate: {opacity: 1},
        exit: {opacity: 0},
  ...
  return (
    <Card ref={cardRef} className="w-full">
      <m.div className="w-full" layout={!reducedMotion}>
        <AnimatePresence initial={false} mode="popLayout">
          {uploadState === "idle" ? (
            <m.div key="empty" className="w-full" layout={!reducedMotion} {...crossfade}>
            ...
            <m.div key="file" className="w-full" layout={!reducedMotion} {...crossfade}>
              <Card.Content className="w-full gap-3">
                {/* The Change rotation crossfades the file row the same way. */}
                <AnimatePresence initial={false} mode="popLayout">
                  <m.div key={fileIndex} className="flex w-full items-center gap-3" {...crossfade}>
