...
function agentApiDocument(origin: string): OpenAPIDocument {
  return {
    ...baseDocument(
      "blakeUI Docs Agent API",
      AGENT_API_VERSION,
      getAgentServiceBaseUrl(origin),
      "Read-only endpoints that help agents search blakeUI documentation and retrieve markdown page content.",
    ),
    paths: {
      "/health": {
        get: {
      ...
      "/page": {
        get: {
          operationId: "getBlakeUIDocPageMarkdown",
          parameters: [
            {
              description: "blakeUI docs page URL, for example /docs/react/components/button",
              in: "query",
              name: "url",
              required: true,
              schema: {type: "string"},
      ...
      "/search": {
        get: {
          ...
          summary: "Search blakeUI documentation",
        },
      },
    },
  };
...
function mcpApiDocument(kind: "react" | "native"): OpenAPIDocument {
  ...
  const title = isReact ? "blakeUI React MCP Data API" : "blakeUI Native MCP Data API";
  ...
  return {
    ...baseDocument(
      ...
      "1.1.0",
      serverUrl,
      `Read-only data API used by the ${isReact ? "@blakeui/react-mcp" : "@blakeui/native-mcp"} package and blakeUI agent skills.`,
    ),
    paths: {
      "/health": {
        get: {
...
const OPENAPI_BUILDERS: Record<string, (origin: string) => OpenAPIDocument> = {
  "blakeui-agent-api.json": agentApiDocument,
  "blakeui-native-mcp-api.json": () => mcpApiDocument("native"),
  "blakeui-react-mcp-api.json": () => mcpApiDocument("react"),
};
