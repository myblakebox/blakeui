 ...
 *
 * Uber is the one preset whose accent differs between modes. Both consumers now
 * read that from the preset's darkAccent: this script uses it directly, and the
 * builder reaches it through adaptiveColors, which theme-values.ts derives from
 * the same field. The tokens overridden here are the ones use-css-sync
 * overrides, minus --accent-soft-foreground, which this stylesheet never emits.
 */
