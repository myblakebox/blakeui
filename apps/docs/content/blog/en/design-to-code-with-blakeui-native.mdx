---
title: "Design to Code with blakeUI Native"
description: "How blakeUI Native connects Figma, code, and AI-assisted mobile development."
date: "2026-05-13"
...
image: "/images/blog/design-to-code-with-blakeui-native.jpg"
draft: true
---
...
blakeUI Native is built for that workflow. The library gives developers a maintained React Native component system, gives designers a shared blakeUI visual language through Figma, and gives AI assistants structured documentation through MCP, llms.txt, and skills.
...
The result is subtle inconsistency. One screen has different spacing. Another uses the wrong disabled state. A generated component imports web blakeUI into a native app. The first version ships, but the system becomes harder to maintain.

blakeUI Native is designed to reduce that drift.

## Shared Mental Model

blakeUI Native uses the same product-level ideas as the rest of the blakeUI ecosystem:
...
The implementation is native, not web. blakeUI Native uses React Native, Uniwind, `BlakeUINativeProvider`, and mobile interaction patterns. But the design language stays familiar.
...
The [blakeUI Figma Kit V3](https://www.figma.com/community/file/1546526812159103429/blakeui-figma-kit-v3) is the design-side entry point for the ecosystem. It is linked from the Native docs so teams can work from the same visual system while building mobile screens.

Use it to:

- Start from blakeUI's visual language instead of blank frames.
- Keep component names and variants close to the code model.
- Align designers and developers on hierarchy, states, and spacing.
- Create mobile product screens that feel connected to the rest of the blakeUI ecosystem.
...
blakeUI Native provides dedicated AI entry points:
...
- [blakeUI Native MCP Server](/docs/native/getting-started/mcp-server), published as `@blakeui/native-mcp`.
- [blakeUI Native Agent Skills](/docs/native/getting-started/agent-skills).

That matters because Native code has different rules:

- Import from `@blakeui/native`.
- Use React Native press handlers such as `onPress`.
- Configure `BlakeUINativeProvider`.
- Wrap the app with `GestureHandlerRootView`.
- Use Uniwind and `@blakeui/native/styles`.
...
1. **Design from the Figma kit.** Start from blakeUI's component language instead of one-off mobile components.
2. **Install blakeUI Native correctly.** Follow the [Quick Start](/docs/native/getting-started/quick-start) so Uniwind, peer dependencies, styles, and providers are configured.
...
6. **Update through the package.** Treat blakeUI Native as a maintained dependency, not a copied snapshot.

## Why This Helps

Design-to-code is not about removing engineers or designers. It is about reducing translation loss.

blakeUI Native helps because:

- Designers start from the blakeUI visual system.
...
Install the [blakeUI Native MCP Server](/docs/native/getting-started/mcp-server) in your editor, then ask your assistant to build a screen using the [Quick Start](/docs/native/getting-started/quick-start) and the components in [All Components](/docs/native/components).
