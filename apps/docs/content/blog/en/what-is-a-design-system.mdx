...
description: "Learn what a design system is, why your team needs one, and how React component libraries like blakeUI make building them easier."
date: "2026-05-08"
...
**Example:** A `Button` component in a package like `@blakeui/react` that accepts `size` and semantic `variant` props.
...
Tailwind CSS v4 uses a CSS-first token system with `@theme`, which blakeUI builds on. blakeUI's shipped theme maps real CSS variables such as `--accent`, `--background`, `--foreground`, `--radius`, and `--surface-shadow` into Tailwind utilities.
...
blakeUI uses this pattern. Each sub-component is independently styleable, and you can omit parts you don't need. The parent component manages shared state (like selection in a RadioGroup) while children control their own rendering.
...
blakeUI builds on [React Aria](https://react-spectrum.adobe.com/react-aria/), one of the most comprehensive accessibility primitive libraries available for React. This means blakeUI components — from Button to Calendar to ComboBox — inherit battle-tested accessibility behavior.
...
|---------|---------|----------|
| blakeUI | Tailwind CSS v4 | Modern stack, accessibility, AI tooling |
...
- **React Aria** — Most comprehensive accessibility primitives. Used by blakeUI.
...
Many teams start by adopting a library (option 1) and gradually extend it. blakeUI supports this through:
...
<div className="rounded-md bg-accent shadow-surface">
  <p className="text-accent-foreground">Styled with BlakeUI tokens</p>
</div>
```
...
blakeUI uses this layered approach. You can override at any level — change the primary color globally, adjust the card radius specifically, or create an entirely new theme.
...
blakeUI provides the component library and token system layers of a design system:
...
2. **Adopt a component library.** Start with blakeUI or another library that matches your stack. Customize the tokens to match your brand.
...
- [blakeUI Quick Start Guide](/docs/react/getting-started/quick-start) — Get started with blakeUI's component library
- [blakeUI Theming Guide](/docs/react/getting-started/theming) — Customize design tokens and themes
- [React Component Libraries Compared](/blog/react-component-library-comparison) — Compare your options
- [Component Documentation](/docs/react/components) — Explore blakeUI's component library
