...
|---------------------|------|----------------------------|
| **blakeUI Pro Dashboard** | Template + Pro components | blakeUI-native app shell, sidebar, navbar, KPI, data grid, and chart patterns |
...
| **blakeUI Pro + custom widgets** | Pro components + template | Start from Pro layout/table/chart primitives, then add product-specific widgets |

---

## 1. blakeUI Pro Dashboard Template

[blakeUI Pro](https://blakeui.com/pro) offers pre-built dashboard templates designed specifically for SaaS products, admin panels, and analytics dashboards. Built on blakeUI components with Tailwind CSS v4.
...
- Dark mode support with blakeUI's theming system
...
- **AI tooling.** Use blakeUI's MCP server with Cursor or other AI tools to extend and customize dashboards with AI assistance.

**Pricing:** Premium license through blakeUI Pro; check the current pricing page for details.
...
## 14. blakeUI Pro + Custom Widgets

If you want a blakeUI-native dashboard, start with [blakeUI Pro](https://blakeui.com/pro) rather than rebuilding the frame from base components. Use the Pro dashboard template for the app shell and add product-specific widgets only where your product needs something unique.

**What you get:**

- `AppLayout`, `Sidebar`, and `Navbar` for the dashboard frame
- `KPI`, `DataGrid`, `LineChart`, `NumberValue`, and `TrendChip` for dashboard UI
- Base blakeUI components for local UI around the Pro primitives
...
- **Modern stack.** The current dashboard template is built with Next.js and blakeUI Pro components.
- **AI-assisted development.** Use blakeUI's MCP server and docs so agents can extend the dashboard with current APIs.

**How to start:**

1. Start from the blakeUI Pro dashboard template.
...
**Pricing:** Check blakeUI Pro's current pricing and license before choosing it.

**Best for:** Teams that want a blakeUI-native dashboard without rebuilding dashboard infrastructure from scratch.
...
- **Speed to launch:** blakeUI Pro Dashboard or Ant Design Pro give you the most complete starting points.
- **Maximum flexibility:** blakeUI Pro plus custom widgets or the shadcn Dashboard example let you build around your product's exact needs.
...
## Build Your Dashboard with blakeUI

blakeUI provides the components you need to build modern, accessible dashboards:
...
- [blakeUI Pro](https://blakeui.com/pro) — Pre-built dashboard templates and premium components
- [Tutorial: Build a React Dashboard](/blog/build-react-dashboard-with-blakeui) — Step-by-step guide
