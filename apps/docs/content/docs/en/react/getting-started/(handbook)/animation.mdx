...
**Respecting motion preferences:** blakeUI automatically respects user motion preferences using Tailwind's `motion-reduce:` utility. All built-in transitions and animations are disabled when users enable "reduce motion" in their system settings.
...
```css
/* BlakeUI pattern - uses Tailwind's motion-reduce: */
.button {
...
blakeUI automatically detects the user's `prefers-reduced-motion: reduce` setting and disables animations accordingly.
