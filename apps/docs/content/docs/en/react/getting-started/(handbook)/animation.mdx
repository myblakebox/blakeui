...
Continuous indicators are the exception. A spinner or indeterminate bar still uses `animate-none`, because a looping motion has no gentler form - and a frozen indicator is a separate design problem worth solving deliberately rather than by sweep.
