...
description: Core principles that guide blakeUI's design and development
icon: book
---

blakeUI follows 10 core principles that prioritize clarity, accessibility, customization, and developer experience.
...
<DocsImage
  src="/images/components/design-principles.png"
...
```tsx
import { Tabs, TabList, Tab, TabPanel } from '@blakeui/react';
...
import { Alert, AlertIcon, CardHeader, AccordionTrigger } from '@blakeui/react';
...
Styles (`@blakeui/styles`) are separate from logic (`@blakeui/react`), enabling use with any framework or vanilla HTML. See [Tailwind Play example](https://play.tailwindcss.com/vMYXzKPyUx).
...
// Apply BlakeUI's BEM classes directly to any element
