...
<VideoPlayer
  src="https://cdn.blakeui.com/videos/heroui-v3-release.mp4"
  height={430}
  muted
/>
...
<VideoPlayer
  src="https://cdn.blakeui.com/videos/heroui-native-v1-release-.mp4"
  playMode="manual"
  height={430}
  muted
/>

[Jump to details](#@blakeui/native)

### blakeUI Pro

<DocsImage
  src="https://cdn.blakeui.com/docs/heroui-pro@2x.jpg"
  darkSrc="https://cdn.blakeui.com/docs/heroui-pro-dark@2x.jpg"
  alt="blakeUI Pro"
  href="https://blakeui.pro"
/>

Premium components, templates, and AI tooling for React and React Native. Command palette, Kanban, DataGrid, Dashboard templates, and more. Pre-sale pricing at [blakeui.pro](https://blakeui.pro).

[Jump to details](#blakeui-pro)
...
**Styles separated from implementation:** `@blakeui/styles` is standalone CSS. `@blakeui/react` handles behavior. Use the styles with React, plain HTML + Tailwind, or any framework. BEM class names make every slot customizable globally. Swap themes to change not just variables, but how components look and feel.

**Headless when you want it:** Remove the `@blakeui/styles` import and you have a headless library. We maintain functionality and accessibility. You focus on your product.
...
@import "@blakeui/styles/components/card.css" layer(components);
```
...
Some users need animations disabled. blakeUI extends Tailwind's `motion-reduce:` variant to support both the system preference and a data attribute:
...
<html data-reduce-motion="true">
  <!-- All BlakeUI animations are disabled -->
</html>
```
...
Framer Motion, Motion One, or any CSS animation library works alongside blakeUI's built-in transitions:
...
// Subpath import
import { Button } from "@blakeui/react/button";
import { Card } from "@blakeui/react/card";
import { Table } from "@blakeui/react/table";
```

## UI for Agents

More developers build by prompting than by reading API docs. blakeUI accounts for that.

### MCP Server

The blakeUI MCP Server connects AI coding assistants (Cursor, Claude Code, VS Code Copilot, Windsurf, Zed) to component docs, props, source code, CSS styles, and theme variables. The AI reads the source of truth directly instead of guessing from training data.
...
Ask your AI assistant "update blakeUI to the latest version" and it will compare versions, review the changelog for breaking changes, and apply the necessary code updates automatically.

### Agent Skills

Installable knowledge packs for Cursor and Claude Code. Component patterns, variant usage, theming instructions, and upgrade guides. Preloaded context so the AI writes correct blakeUI code on the first attempt.

### LLMs.txt

Structured documentation files optimized for AI context windows. Published at `/llms.txt` and `/llms-components.txt`, these give any LLM-powered tool a machine-readable summary of blakeUI's API surface.

MCP server, agent skills, LLMs.txt. Three layers that give AI assistants the same access to blakeUI that human developers get from docs.
...
blakeUI Native is a brand-new library shipping alongside v3 for the web. Different rendering engine, same mental model. Where the platforms diverge, the APIs adapt to feel native on each.
...
<div className="flex justify-center">
  **[📱 Open Demo App in Expo Go](https://link.blakeui.com/native-demo)**
</div>
...
```tsx
import { Dialog, Button } from "@blakeui/native";
...
If you know blakeUI on the web, most of that knowledge carries over. Familiar component names, dot notation, and prop patterns wherever possible. Where the platforms diverge (layout primitives, gestures, navigation), the APIs adapt to feel native. The mental model stays the same:
...
<VideoPlayer
  src="https://cdn.blakeui.com/videos/native-v1-demo/v-1-demo-2.mp4"
  height={430}
  muted
/>
...
// App-wide
<BlakeUINativeProvider config={{ animation: "disable-all" }}>
  <App />
...
<VideoPlayer
  src="https://cdn.blakeui.com/videos/native-v1-demo/v-1-demo-3.mp4"
  height={430}
  muted
/>
...
blakeUI Native ships with its own MCP Server, agent skills, and LLMs.txt. Same tooling structure as the web library:
...
Alongside v3, the pre-sale of [blakeUI Pro](https://blakeui.pro) is live. Premium components, templates, and AI tooling for both React and React Native.
...
[See plans and pricing at blakeui.pro](https://blakeui.pro)
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm install @blakeui/native
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @blakeui/native
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @blakeui/native
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @blakeui/native
    ```
  </Tab>
</Tabs>
...
Every component in blakeUI has a 1:1 match in Figma. Same variants, same naming, same structure. The kit uses auto layout throughout, Figma variables that map directly to code tokens (`--accent`, `--surface`, `--radius`), and Figma's new [slots](https://help.figma.com/hc/en-us/articles/38231200344599-Use-slots-to-build-flexible-components-in-Figma) for flexible component composition. Designers rearrange, swap, and customize parts the same way developers do in code.

<DocsImage
  src="https://cdn.blakeui.com/docs/cover-figma@2x.jpg"
  darkSrc="https://cdn.blakeui.com/docs/cover-figma-dark@2x.jpg"
  alt="blakeUI Figma Kit v3"
  href="https://www.figma.com/community/file/1546526812159103429/blakeui-figma-kit-v3"
/>

[Get the Figma Kit](https://www.figma.com/community/file/1546526812159103429/blakeui-figma-kit-v3)
...
- [GitHub Repository](https://github.com/blakeui/blakeui)
- [Figma Kit V3](https://www.figma.com/community/file/1546526812159103429/blakeui-figma-kit-v3)
