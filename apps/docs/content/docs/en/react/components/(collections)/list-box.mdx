...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("ListBox")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { ListBox } from '@blakeui/react';
```
...
```tsx
import { ListBox, Label, Description, Header } from '@blakeui/react';
...
The ListBox component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/list-box.css)):
...
```tsx
import { ListBox, Label, Description } from '@blakeui/react';
...
<ListBox aria-label="Users" selectionMode="single">
  ...
  <ListBox.Item id="2" textValue="Alice">
    <Label>Alice</Label>
    <Description>alice@blakeui.com</Description>
  </ListBox.Item>
</ListBox>
```
...
```tsx
import { ListBox, Header, Separator } from '@blakeui/react';
...
```tsx
import { ListBox, Selection } from '@blakeui/react';
...
```tsx
import { ListBox, ListBoxItemIndicator } from '@blakeui/react';
