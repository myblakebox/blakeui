...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("TagGroup")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { TagGroup } from '@blakeui/react';
```
...
```tsx
import { TagGroup, Tag, Label } from '@blakeui/react';
...
The TagGroup component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/tag-group.css) and [tag.css](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/tag.css)):
