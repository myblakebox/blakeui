...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("Form")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { Form } from '@blakeui/react';
```
...
```tsx
import {Form, Button} from '@blakeui/react';
...
```tsx
import {Form, TextField, Label, Input, FieldError, Button} from '@blakeui/react';
...
The Form component works seamlessly with blakeUI's form field components:
