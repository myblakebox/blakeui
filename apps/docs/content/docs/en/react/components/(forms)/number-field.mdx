...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("NumberField")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { NumberField } from '@blakeui/react';
```
...
```tsx
import {NumberField, Label, Description, FieldError} from '@blakeui/react';
...
```tsx
import {NumberField, Label} from '@blakeui/react';
