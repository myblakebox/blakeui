...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("InputOTP")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { InputOTP } from '@blakeui/react';
```
...
Use the `pattern` prop to restrict input to specific characters. blakeUI exports common patterns like `REGEXP_ONLY_CHARS` and `REGEXP_ONLY_DIGITS`.
...
The InputOTP component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/input-otp.css)):
...
blakeUI re-exports common regex patterns from input-otp for convenience:

```tsx
import { REGEXP_ONLY_DIGITS, REGEXP_ONLY_CHARS, REGEXP_ONLY_DIGITS_AND_CHARS } from '@blakeui/react';
