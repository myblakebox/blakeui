...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("TextField")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { TextField } from '@blakeui/react';
```
...
```tsx
import {TextField, Label, Input, Description, FieldError} from '@blakeui/react';
...
- **Input** - Single-line text input from `@blakeui/react`
- **TextArea** - Multi-line text input from `@blakeui/react`
