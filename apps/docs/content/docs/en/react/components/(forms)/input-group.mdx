...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("InputGroup")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { InputGroup } from '@blakeui/react';
```
...
```tsx
import {InputGroup, TextField, Label} from '@blakeui/react';
...
```tsx
import {InputGroup, TextField, Label, Button} from '@blakeui/react';
...
import {Description, FieldError, InputGroup, Label, TextField} from "@blakeui/react";
