...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("RadioGroup")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { RadioGroup, Radio } from '@blakeui/react';
```
...
```tsx
import {RadioGroup, Radio, Label, Description, FieldError} from '@blakeui/react';
...
The RadioGroup component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/radio-group.css)):
