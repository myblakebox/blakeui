...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("ColorPicker")` and build the behaviour yourself.
</Callout>
...
```tsx
import { ColorPicker, ColorArea, ColorSlider, ColorSwatch, Label } from '@blakeui/react';
...
The ColorPicker component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/color-picker.css)):
