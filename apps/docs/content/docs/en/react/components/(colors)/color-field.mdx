...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("ColorField")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { ColorField, parseColor } from '@blakeui/react';
```
...
```tsx
import {ColorField, Label, ColorSwatch, Description, FieldError, parseColor} from '@blakeui/react';
...
```tsx
import {ColorField, Label, ColorSwatch, Description} from '@blakeui/react';
...
- **Label** - Field label component from `@blakeui/react`
...
- **ColorSwatch** - Color preview component from `@blakeui/react`
- **Description** - Helper text component from `@blakeui/react`
- **FieldError** - Validation error message from `@blakeui/react`
