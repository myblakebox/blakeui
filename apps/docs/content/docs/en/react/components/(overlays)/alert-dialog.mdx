...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("AlertDialog")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { AlertDialog } from "@blakeui/react";
```
...
The AlertDialog component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/alert-dialog.css)):
...
```tsx
import {useOverlayState} from "@blakeui/react";
