...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("Modal")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { Modal } from "@blakeui/react";
```
...
The Modal component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/modal.css)):
