...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("Drawer")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { Drawer, Button } from "@blakeui/react";
```
...
The Drawer component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/drawer.css)):
