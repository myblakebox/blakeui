...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("Slider")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { Slider } from '@blakeui/react';
```
...
```tsx
import { Slider, Label } from '@blakeui/react';
...
The Slider component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/slider.css)):
