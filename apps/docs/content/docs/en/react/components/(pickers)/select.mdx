...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("Select")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { Select } from "@blakeui/react";
```
...
```tsx
import {Select, Label, Description, Header, ListBox, Separator} from "@blakeui/react";
...
The Select component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/select.css)):
