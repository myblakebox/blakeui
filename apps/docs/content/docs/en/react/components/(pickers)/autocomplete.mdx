...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("Autocomplete")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { Autocomplete, useFilter } from "@blakeui/react";
```
...
```tsx
import {Autocomplete, Label, Description, SearchField, ListBox} from "@blakeui/react";
...
```tsx
import {Autocomplete, SearchField, ListBox} from "@blakeui/react";
...
The Autocomplete component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/autocomplete.css)):
...
```tsx
import {useFilter} from "@blakeui/react";
