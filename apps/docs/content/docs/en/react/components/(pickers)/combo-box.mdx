...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("ComboBox")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { ComboBox } from '@blakeui/react';
```
...
```tsx
import { ComboBox, Input, Label, Description, Header, ListBox, Separator } from '@blakeui/react';
...
```tsx
import { ComboBox, Input } from '@blakeui/react';
...
The ComboBox component uses these CSS classes ([View source styles](https://github.com/myblakebox/BlakeUI/blob/main/packages/styles/components/combo-box.css)):
...
```tsx
import { ComboBox, Input, Label, ListBox, Header, Separator } from '@blakeui/react';
...
```tsx
import { Collection, ComboBox, EmptyState, Input, Label, ListBox, ListBoxLoadMoreItem, Spinner } from '@blakeui/react';
...
```tsx
import { ComboBox, FieldError, Input, Label, ListBox } from '@blakeui/react';
