...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("DateField")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { DateField } from '@blakeui/react';
```
...
```tsx
import {DateField, Label, Description, FieldError} from '@blakeui/react';
...
- **DateField.Input** - Input component with segmented editing from `@blakeui/react`
