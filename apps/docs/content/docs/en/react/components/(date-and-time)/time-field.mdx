...
<Callout title="Completeness: behavior-required" type="warn">
  ...
  server with `get_component_behavior("TimeField")` and build the behaviour yourself.
</Callout>

## Import

```tsx
import { TimeField } from '@blakeui/react';
```
...
```tsx
import {TimeField, Label, Description, FieldError} from '@blakeui/react';
...
```tsx
import {TimeField, Label, Description} from '@blakeui/react';
...
- **TimeField.Input** - Input component with segmented editing from `@blakeui/react`
