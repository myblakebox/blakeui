...
BlakeUI is a modern React UI library built with Tailwind CSS v4, using a pnpm monorepo structure managed by Turborepo.
...
pnpm build --filter=@blakeui/react
...
- Use `--filter` flag with package name: `pnpm build --filter=@blakeui/react`
- Main packages: `@blakeui/react`, `@blakeui/docs`, `@blakeui/storybook`
...
**IMPORTANT — No AI attribution**: Do not sign commits or PRs with any AI/Claude
attribution. Never add `Co-Authored-By: Claude …`, "Generated with Claude Code",
or similar trailers/footers to commit messages or pull request descriptions.
...
**IMPORTANT**: BlakeUI uses BEM (Block Element Modifier) style for CSS classes to ensure predictable and maintainable styling:
...
- The `button` component has been migrated to use CSS styles from `@blakeui/styles/src/components/button.css`
...
**IMPORTANT**: BlakeUI follows a compound component pattern similar to Radix UI, built on top of React Aria Components primitives. This enables maximum flexibility and customization for users.
...
1. **Styling with Tailwind Variants**:
   ...
   - **IMPORTANT**: Always import from `tailwind-variants`, never from `@blakeui/standard` (which doesn't exist)
...
2. **Component Features**:
   ...
   - Display names follow: `BlakeUI.ComponentName` or `BlakeUI.Component.SubPart`
...
6. **Composition Pattern with Existing Components**:
   ...
   **CRITICAL**: BlakeUI follows a composition-based approach. Components should reuse existing primitives rather than creating component-specific versions.
...
3. **Review & Correction Phase**:
   ...
   - Confirm the plan aligns with BlakeUI patterns and conventions
...
1. **Creating New Components**:
   ...
   **Component Creation - ALWAYS USE THE SCRIPT**:
   ...
   - Generate boilerplate following BlakeUI patterns
   ...
   **Implementation Steps**:
   - Study existing BlakeUI components (accordion, alert) to understand the compound pattern
...
3. **Documentation**:
   ...
   - Uses MDX format
   - BlakeUI components are pre-imported
...
**IMPORTANT**: BlakeUI uses Iconify with gravity-ui as the default icon set.
...
3. **Monorepo Configurations**:
   ...
   - Study monorepo setups similar to BlakeUI
...
When working with BlakeUI CSS components, follow these critical patterns:
