...
```tsx
import { InputOTP } from '@blakeui/native';
```
...
```tsx
import { InputOTP, REGEXP_ONLY_CHARS } from '@blakeui/native';
...
```tsx
import { InputOTP, useBottomSheetAwareHandlers } from '@blakeui/native';
...
```tsx
import { InputOTP, Label, Description, type InputOTPRef } from '@blakeui/native';
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/input-otp.tsx>).
