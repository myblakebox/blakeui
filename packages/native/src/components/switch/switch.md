...
```tsx
import { Switch } from '@blakeui/native';
```
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/switch.tsx>).
...
```tsx
import { useSwitch } from '@blakeui/native';
...
```tsx
import { Description, ControlField, Label } from '@blakeui/native';
