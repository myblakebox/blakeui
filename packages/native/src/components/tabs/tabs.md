...
```tsx
import { Tabs } from '@blakeui/native';
```
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/tabs.tsx>).
...
```tsx
import { useTabs } from '@blakeui/native';
...
```tsx
import { useTabsMeasurements } from '@blakeui/native';
...
```tsx
import { useTabsTrigger } from '@blakeui/native';
