...
```tsx
import { TextArea } from '@blakeui/native';
```
...
```tsx
import { Description, Label, TextArea, TextField } from '@blakeui/native';
...
```tsx
import { FieldError, Label, TextArea, TextField } from '@blakeui/native';
...
```tsx
import { Label, TextArea, TextField } from '@blakeui/native';
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/text-area.tsx>).
