...
```tsx
import { Button, PressableFeedback } from '@blakeui/native';
...
```tsx
import { Button, useThemeColor } from '@blakeui/native';
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/button.tsx>).
...
```tsx
import { useButton } from '@blakeui/native';
