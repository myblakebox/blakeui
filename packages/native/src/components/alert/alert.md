...
```tsx
import { Alert } from '@blakeui/native';
```
...
```tsx
import { Alert, Button, CloseButton } from '@blakeui/native';
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/alert.tsx>).
...
```tsx
import { useAlert } from '@blakeui/native';
