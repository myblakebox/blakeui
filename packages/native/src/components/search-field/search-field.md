...
```tsx
import { SearchField } from '@blakeui/native';
```
...
```tsx
import { Description, Label, SearchField } from '@blakeui/native';
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/search-field.tsx>).
...
```tsx
import { useSearchField } from '@blakeui/native';
