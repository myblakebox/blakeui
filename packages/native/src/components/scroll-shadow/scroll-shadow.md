...
```tsx
import { ScrollShadow } from '@blakeui/native';
```
...
```tsx
import { ScrollShadow, Surface } from '@blakeui/native';
import { LinearGradient } from 'expo-linear-gradient';
...
You can find more examples in the [GitHub repository](<https://github.com/myblakebox/BlakeUI/blob/main/example/src/app/(home)/components/scroll-shadow.tsx>).
