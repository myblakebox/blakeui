...
BlakeUI Native components provide flexible styling options: Tailwind CSS utilities, StyleSheet API, and render props for dynamic styling.

## Styling Principles

BlakeUI Native is built with `className` as the go-to styling solution. You can use Tailwind CSS classes via the `className` prop on all components.
...
**Using className:** All BlakeUI Native components accept `className` props:
...
```tsx
import { RadioGroup, Label, cn } from '@blakeui/native';
...
```tsx
import { Button } from '@blakeui/native';
import type { ButtonRootProps } from '@blakeui/native';
...
Each BlakeUI Native component exports a `classNames` object that contains the same styling functions used internally by the component. This is particularly useful when you want to style your own custom components to match the appearance of BlakeUI Native components.

For example, you can style a custom `Link` component to look like a `Button`:

```tsx
import { buttonClassNames, cn } from '@blakeui/native';
...
```tsx
import { buttonClassNames } from '@blakeui/native';
...
The `classNames` functions accept the same variant props as the components themselves, allowing you to maintain visual consistency across your custom components and BlakeUI Native components.

## Responsive Design

BlakeUI Native supports Tailwind's responsive breakpoint system via [Uniwind](https://docs.uniwind.dev/breakpoints). Use breakpoint prefixes like `sm:`, `md:`, `lg:`, and `xl:` to apply styles conditionally based on screen width.
...
BlakeUI Native provides utility functions to assist with styling components.
