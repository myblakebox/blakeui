...
BlakeUI Native uses CSS variables for theming. Customize everything from colors to component styles using standard CSS.

## How It Works

BlakeUI Native's theming system is built on top of [Tailwind CSS v4](https://tailwindcss.com/docs/theme)'s theme via [Uniwind](https://uniwind.dev/). When you import `@blakeui/native/styles`, it uses Tailwind's built-in color palettes, maps them to semantic variables, automatically switches between light and dark themes, and uses CSS layers and the `@theme` directive for organization.
...
BlakeUI Native automatically supports dark mode through [Uniwind](https://docs.uniwind.dev/theming/basics). The theme switches between light and dark variants based on system preferences or manual selection:
...
export default function App() {
  ...
  return <BlakeUINativeProvider>{/* Your app content */}</BlakeUINativeProvider>;
}
```
...
All BlakeUI Native components automatically use these font variables, ensuring consistent typography throughout your app.

## Variables Reference

BlakeUI defines three types of variables:
