 ...
 * Toast Queue
 * --------------------------------------------------------------------------------------------- */
/** The underlying react-stately queue passed to `ToastRegion` (not the BlakeUI `ToastQueue` wrapper). */
...
export interface BlakeUIToastOptions {
...
function createToastFunction(queue: ToastQueue<ToastContentValue>) {
  const toastFn = (message: ReactNode, options?: BlakeUIToastOptions): string => {
  ...
  toastFn.success = (
    message: ReactNode,
    options?: Omit<BlakeUIToastOptions, "variant">,
  ): string => {
    return toastFn(message, {...options, variant: "success"});
  };

  toastFn.danger = (message: ReactNode, options?: Omit<BlakeUIToastOptions, "variant">): string => {
    return toastFn(message, {...options, variant: "danger"});
  };

  toastFn.info = (message: ReactNode, options?: Omit<BlakeUIToastOptions, "variant">): string => {
