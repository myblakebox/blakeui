...
/* -------------------------------------------------------------------------------------------------
 * Table With Filters — composition showcase
 *
 * A filter toolbar composed ABOVE the restyled Table primitive. Everything here is composition in a
 * story: the Table component itself is untouched. The toolbar is a plain flex row rendered as the
 * first child of <Table> (which is a styled card div), so it sits inside the floating card above the
 * default-color header pill. Structure/behaviour reference: HeroUI v2 Table "Use Case Example".
 * -----------------------------------------------------------------------------------------------*/
...
export default {
  ...
  title: "Components/Data Display/Table With Filters",
} as Meta<typeof Table>;

type Story = StoryObj<typeof Table>;

/* -------------------------------------------------------------------------------------------------
 * Sample Data (mirrors the Default Table story dataset)
 * -----------------------------------------------------------------------------------------------*/
interface User {
  id: number;
  name: string;
 ...
 * Column definitions
 *
 * Both the header columns and the body cells are driven from this single list (filtered by
 * visibleColumns). Rendering header + body from the same source via Table.Collection guarantees the
 * cell count always matches the column count — the supported React Aria pattern for dynamic columns.
 * -----------------------------------------------------------------------------------------------*/
interface ColumnDef {
  id: string;
  label: string;
...
function TableWithFiltersTemplate() {
  ...
  return (
    <div className="w-full max-w-4xl">
      <Table>
        {/* Toolbar — single row: search constrained left, action group pushed right. */}
...
/**
 * Table with a composed filter toolbar: live search, multi-select Status & Role filters, column
 * visibility, and an "Add New" action — over a filtered / sorted / paginated dataset.
 */
