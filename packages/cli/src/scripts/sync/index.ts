...
export function syncDocs() {
  ...
  const matchDocs = docs.match(/(?<=Usage: blakeui \[command]\n\n)[\W\w]+(?=## Documentation)/)?.[0];

  const targetPath = resolver('blakeui/apps/docs/content/docs/api-references/cli-api.mdx');
  const targetDocs = readFileSync(targetPath, 'utf-8');
  const replaceTargetDocs = targetDocs.replace(/(?<=Usage: blakeui \[command])[\W\w]+/, '');
...
function syncApiRoutes() {
  const targetPath = resolver('blakeui/apps/docs/config/routes.json');
