...
export async function upgrade<T extends Upgrade = Upgrade>(options: ExtractUpgrade<T>) {
  const {all, allDependencies, isBlakeUIAll, upgradeOptionList} = options as Required<Upgrade>;
  let result: UpgradeOption[] = [];
  const missingDepSet = new Set<MissingDepSetType>();

  const allOutputData = await getAllOutputData(all, isBlakeUIAll, allDependencies, missingDepSet);
...
export async function getAllOutputData(
  ...
  const latestVersion = await getConditionVersion(BLAKE_UI);

  const {currentVersion, versionMode} = getVersionAndMode(allDependencies, BLAKE_UI);
  ...
  const blakeUIPeerDepList = await getPackagePeerDep(BLAKE_UI, allDependencies, missingDepSet);
  ...
  const allPeerDepList = [...blakeUIPeerDepList];
  const allOutputData = {
    allOutputList,
    allPeerDepList
  };
