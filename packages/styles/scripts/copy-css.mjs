...
// Copy font assets (self-hosted woff2/woff/ttf/otf) referenced by @font-face
const fontsSrcPath = path.join(rootDir, "fonts");

if (fs.existsSync(fontsSrcPath)) {
  const fontsDistPath = path.join(distDir, "fonts");

  if (!fs.existsSync(fontsDistPath)) {
    fs.mkdirSync(fontsDistPath, {recursive: true});
  }

  for (const file of fs.readdirSync(fontsSrcPath)) {
    if (/\.(woff2?|ttf|otf)$/i.test(file)) {
      fs.copyFileSync(path.join(fontsSrcPath, file), path.join(fontsDistPath, file));
