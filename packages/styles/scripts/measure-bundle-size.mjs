...
async function main() {
  ...
  console.log("BlakeUI CSS Bundle Size Measurement");
  console.log("=".repeat(60));
  console.log();
  ...
  if (compareOnly && previous) {
    ...
    console.log("Minified (blakeui.min.css):", formatBytes(previous.minified.bytes).formatted);
    console.log();
  ...
  const minifiedPath = path.join(distDir, "blakeui.min.css");
  ...
  console.log(
    `Minified (blakeui.min.css):  ${result.minified.formatted}${previous ? showComparison(minifiedSize, previous.minified.bytes, "minified") : ""}`,
  );
  console.log(
