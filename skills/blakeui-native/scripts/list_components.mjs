#!/usr/bin/env node
/**
 * List all available BlakeUI Native components.
 *
 * Usage:
...
const API_BASE = process.env.BLAKEUI_NATIVE_API_BASE || "https://native-mcp-api.blakeui.com";
const APP_PARAM = "app=native-skills";
const LLMS_TXT_URL = "https://blakeui.com/native/llms.txt";

/**
 * Fetch data from BlakeUI Native API with app parameter for analytics.
 */
async function fetchApi(endpoint) {
  ...
  try {
    const response = await fetch(url, {
      headers: {"User-Agent": "BlakeUI-Native-Skill/1.0"},
...
async function fetchFallback() {
  try {
    ...
    for (const line of content.split("\n")) {
      ...
      // Match: - [ComponentName](https://www.blakeui.com/docs/native/components/component-name)
      // Skip "All Components" which links to /components without a specific component
      if (inComponentsSection) {
        const match = line.match(
          /^\s*-\s*\[([^\]]+)\]\(https:\/\/www\.blakeui\.com\/docs\/native\/components\/[a-z]/,
        );
...
/**
 * Main function to list all available BlakeUI Native components.
 */
async function main() {
