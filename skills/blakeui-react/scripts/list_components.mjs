#!/usr/bin/env node
/**
 * List all available BlakeUI components.
 *
 * Usage:
...
const API_BASE = process.env.BLAKEUI_API_BASE || "https://mcp-api.blakeui.com";
const APP_PARAM = "app=react-skills";
const LLMS_TXT_URL = "https://blakeui.com/react/llms.txt";

/**
 * Fetch data from BlakeUI API with app parameter for analytics.
 */
async function fetchApi(endpoint) {
  ...
  try {
    const response = await fetch(url, {
      headers: {"User-Agent": "BlakeUI-Skill/1.0"},
...
async function fetchFallback() {
  try {
    ...
    for (const line of content.split("\n")) {
      ...
      // Match: - [ComponentName](https://www.blakeui.com/docs/react/components/component-name)
      // Skip "All Components" which links to /components without a specific component
      if (inComponentsSection) {
        const match = line.match(
          /^\s*-\s*\[([^\]]+)\]\(https:\/\/www\.blakeui\.com\/docs\/react\/components\/[a-z]/,
        );
...
/**
 * Main function to list all available BlakeUI components.
 */
async function main() {
